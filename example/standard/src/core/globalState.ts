...
import type { AppKey } from './loader';
...
export const createAtom = <TState>(config: Omit<Atom<TState>, 'key'>) =>
  ({
    key: keyCounter++,
    ...config,
  }) as Atom<TState>;
...
export class GlobalState {
  ...
  reset = (): void =>
    ReactDOM.unstable_batchedUpdates(() => this._globalState.forEach((v) => v.resetValue(true)));
}
...
export const useGlobalStateContext = (): GlobalState => {
  ...
  if (globalState == null)
    throw new Error('You should add <GlobalStateContext.Provider> as the root component');
  return globalState;
};
...
export const useSetAtom = <T>(atom: Atom<T>): Dispatch<SetStateAction<T>> =>
  useGlobalStateContext().get(atom).setValue;

export const useResetAtom = <T>(atom: Atom<T>): (() => void) =>
  useGlobalStateContext().get(atom).resetValue;
